...
import CryptoPriceMonitor from "../pages/CryptoPriceMonitor";
import { Asset, listAssets } from "../services/coinapi.services";
import axios from "axios";

jest.mock('axios');

describe("<CryptoPriceMonitor />", () => {
    test("should display default crypto price info", async () => {
        ...
        const { findByTestId } = render(<CryptoPriceMonitor />)
        const titleEle = findByTestId('title')
        // const assetsEle = findByTestId('assets')

        expect((await titleEle).innerHTML).toBe('Cryptocurrency Realtime Price')

        await Promise.all(filter_asset_id.map(async asset_id => {
            await waitFor(async () => {
                expect(await findByTestId(asset_id)).toBeInTheDocument();
            }, {
                timeout: 3000,
            });
        }))
